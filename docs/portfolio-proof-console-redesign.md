...
- `src/hooks/usePortfolioMotion.ts` owns the GSAP timeline, ambient console motion, desktop pointer tilt, ScrollTrigger section tracking, and all teardown logic.
...
The motion system uses `gsap.matchMedia()` to keep desktop-only interaction and reduced-motion behavior scoped. All animations are limited to opacity and transforms. React Strict Mode and responsive media changes are handled through GSAP cleanup and event-listener teardown.
...
- The first motion iteration intentionally excludes pinned case-study storytelling and WebGL. Those remain follow-ups and should be implemented only with real case-study media and a poster/fallback path.
